...
import { Upload, FileText, CheckCircle2, X, AlertCircle, ArrowRight, FolderPlus, FileSpreadsheet, Check } from 'lucide-react';
  ...
  const Dropzone = ({ type, label, data }: { type: 'osm' | 'google', label: string, data: any[] }) => (
    <div className="flex-1 group">
      ...
      <div className={`relative min-h-[320px] rounded-[2rem] p-8 flex flex-col items-center justify-center transition-all duration-500 border-2 border-dashed ${
        data.length > 0 
          ? 'bg-emerald-50/30 border-emerald-100 shadow-inner' 
        ...
        {data.length > 0 ? (
          <div className="flex flex-col items-center text-center animate-in zoom-in-95 duration-500">
            ...
            <p className="font-extrabold text-slate-800 text-lg mb-1">Dataset Loaded</p>
    ...
    <div className="max-w-5xl mx-auto">
      <div className="mb-12">
        <div className="flex items-center gap-4 mb-3">
          <div className="px-3 py-1 bg-indigo-50 rounded-lg text-[10px] font-black text-indigo-500 uppercase tracking-widest">Workspace Active</div>
        </div>
        <h2 className="text-4xl font-black text-slate-800 tracking-tight">{job.name}</h2>
        <p className="text-slate-500 font-medium mt-2 max-w-2xl">{job.description || 'Import your data sources to begin the matching engine.'}</p>
      </div>
      ...
      <div className="flex flex-col items-center pt-8 border-t border-slate-50">
        ...
        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-6">Ready to match {(job.osmData.length * job.googleData.length).toLocaleString()} potential pairs</p>
      </div>
    </div>
  );
};
